	...
	<div class="col-md-12">
		<div class="card mr-4">
			...
			<div class="card-body">
				<div class="container-fluid">
					<div class="row">
						<p><b>LEGEND :</b> <span class="legend badge badge-primary">&nbsp;</span> Your selected option.  <span class="legend badge badge-success">&nbsp;</span> Correct Answer.</p>
					</div>
				<?php
					$i = 1;
							$ppoints = 0;
					$qquery = $conn->query("SELECT * FROM questions where md5(qid) = '".$_GET['quiz']."' order by order_by asc");
					if($qquery->num_rows > 0){
						$qry2 = $conn->query("SELECT * FROM question_opt where question_id in (SELECT id FROM questions where md5(qid) = '".$_GET['quiz']."' ) ");
						...
						$answers = $conn->query("SELECT * FROM answers where md5(quiz_id) = '".$_GET['quiz']."' and user_id =".$_SESSION['login_id']);
